...
execute if score Time TICK matches -1 if score Time SECOND matches 1..60 run title @a title [{"text":"潜伏時間","color":"red","bold":true}]
...
## Sound System
execute as @a if score Time TICK matches -1 if score Time SECOND matches 4..10 run playsound minecraft:item.trident.return master @s ~ ~ ~ 1 1 1
execute as @a if score Time TICK matches -1 if score Time SECOND matches 1..3 run playsound minecraft:item.trident.throw master @s ~ ~ ~ 1 1 1
