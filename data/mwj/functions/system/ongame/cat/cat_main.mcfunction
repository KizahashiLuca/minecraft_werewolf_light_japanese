...
execute if score @s DEATH matches 2 if score @s NUM = @p[tag=StrayBullet] STRAY_BY_CAT if score @p[tag=StrayBullet] DEATH matches 1 if score @p[tag=StrayBullet] ROLE matches 1..2 run scoreboard players set @s CAT_ACHIEVE1 1
execute if score @s DEATH matches 2 if score @s NUM = @p[tag=StrayBullet] STRAY_BY_CAT run tellraw @p[tag=StrayBullet] ["",{"text":"----------------------------------\n","color":"white"}]
