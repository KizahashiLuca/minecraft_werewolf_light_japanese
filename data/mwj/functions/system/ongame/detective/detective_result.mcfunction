...
## Error Message
execute if score @p[scores={TEMP2=1}] DEATH matches 1..2 unless score @p[scores={TEMP2=1}] TEMP1 matches 0..60 run tellraw @p[scores={ROLE=8}] ["",{"selector":"@p[scores={TEMP2=1}]","color":"reset"},{"text":" は行方不明です。","color":"reset"}]

## Compare Victim Num and Killer Num
execute if score @p[scores={TEMP2=1}] DEATH matches 1..2 if score @p[scores={TEMP2=1}] TEMP1 matches 0..60 as @a if score @p[scores={TEMP2=1}] VICTIM = @s KILLER run tellraw @p[scores={ROLE=8}] ["",{"selector":"@p[scores={TEMP2=1}]","color":"reset"},{"text":" は ","color":"reset"},{"selector":"@s","color":"reset","bold":true},{"text":" に殺されたことが分かりました。","color":"reset"}]
