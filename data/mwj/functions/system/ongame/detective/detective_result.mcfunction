...
tellraw @p[scores={ROLE=8,ROLE_OF_NUM=1}] ["",{"text":"  探偵結果","color":"reset"}]

## Send a Message that Victim is Alive
execute if score @s DEATH matches 0 run tellraw @p[scores={ROLE=8,ROLE_OF_NUM=1}] ["",{"text":"    ","color":"reset"},{"selector":"@s","color":"reset"},{"text":" は行方不明です。","color":"reset"}]

## Send a Message that Victim Time Out of Range
execute if score @s DEATH matches 1..2 unless score @s TEMP1 matches 0..60 run tellraw @p[scores={ROLE=8,ROLE_OF_NUM=1}] ["",{"text":"    ","color":"reset"},{"selector":"@s","color":"reset"},{"text":" は行方不明です。","color":"reset"}]

## Send a Message that Killer is Known
execute if score @p[scores={TEMP2=1}] DEATH matches 1..2 if score @p[scores={TEMP2=1}] TEMP1 matches 0..60 as @a if score @s NUM = @p[scores={TEMP2=1}] KILLER_NUM run tellraw @p[scores={ROLE=8,ROLE_OF_NUM=1}] ["",{"text":"    ","color":"reset"},{"selector":"@p[scores={TEMP2=1}]","color":"reset"},{"text":" は ","color":"reset"},{"selector":"@s","color":"reset","bold":true},{"text":" に殺されたことが分かりました。","color":"reset"}]
