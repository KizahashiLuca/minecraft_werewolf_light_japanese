...
execute as @a if score @p[scores={CurrentRole=8,RoleOfNum=1}] RoleTarget = @s NUM unless score @p[scores={CurrentRole=8,RoleOfNum=1}] NUM = @s NUM run tellraw @p[scores={CurrentRole=8,RoleOfNum=1}] ["",{"text":"    ","color":"white"},{"selector":"@s","color":"white"},{"text":" > ","color":"white"},{"text":"探偵","clickEvent":{"action":"run_command","value":"/trigger RoleTrigger set 1"},"color":"green","bold":true}]
scoreboard players add @s RoleTarget 1
