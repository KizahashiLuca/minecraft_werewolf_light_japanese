...
execute as @a if score @p[scores={CurrentRole=26,TagOfRoleWork=1}] TargetOfRole = @s NumOfPlayers unless score @p[scores={CurrentRole=26,TagOfRoleWork=1}] NumOfPlayers = @s NumOfPlayers run tellraw @p[scores={CurrentRole=26,TagOfRoleWork=1}] ["",{"text":"    ","color":"white"},{"selector":"@s","color":"white"},{"text":" > ","color":"white"},{"text":"予言","clickEvent":{"action":"run_command","value":"/trigger RoleTrigger set 1"},"hoverEvent":{"action":"show_text","value":[{"text":"予言"}]},"color":"green","bold":true}]
scoreboard players add @s TargetOfRole 1
